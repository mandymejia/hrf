# Quick HPC sanity check: run fit_bestHRF on MOTOR_LR (max subjects, population mode)
# for subject 1 and save the working + population beta plots as PNGs.
#
# Run from the hrf-z repo root on HPC:
#   Rscript dev/hpc/plot_motor_population_betas.R
...
ciftiTools.setOption(
  ...
  if (Sys.info()[['sysname']] == 'Darwin')
    '/Applications/workbench/bin_macosxub/wb_command'
  else
    '~/workbench/bin_rh_linux64/wb_command'
)
