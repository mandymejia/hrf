...
# HPC paths (uncomment for HPC execution)
# save_dir <- "/N/project/hrf_adaptation/validation_bonferroni_0.05/regularize_allHRFs"
# output_dir <- "/N/project/hrf_adaptation/validation_bonferroni_0.05/cm_poc_test45"
# fixture_dir <- "/N/u/zzahid/Quartz/Documents/Github/hrf-z/dev/exploration/fixtures"

# Local paths (comment out for HPC execution)
save_dir <- "/Volumes/LaCie/root/hrf_adaptation/validation_bonferroni_0.05/regularize_allHRFs"
output_dir <- "/Volumes/LaCie/root/hrf_adaptation/validation_bonferroni_0.05/cm_poc_test45"
fixture_dir <- "~/Documents/Github/hrf-z/dev/exploration/fixtures"

# Function to remap paths from HPC to mounted directory (only for local execution)
...
for(r in 1:2){
  ...
  # Load TEST session data (45 matched subjects)
  session_data_test45 <- readRDS(file.path(fixture_dir, paste0('test_matched_session_data_', sess, '.rds')))

  # Remap paths for sshfs mount (only for local execution, comment out for HPC)
  session_data_test45$BOLD_files <- lapply(session_data_test45$BOLD_files, remap_path)
  session_data_test45$nuisance_files <- lapply(session_data_test45$nuisance_files, remap_path)
  ...
  cat("\n********** Processing", sess, "TEST (45 subjects) **********\n"); flush.console()
  ...
  # Collect results for TEST subjects (45 matched)
  results_list <- list()
  n_test_subjects <- length(session_data_test45$BOLD_files)
  cat("Testing on", n_test_subjects, "test subjects\n"); flush.console()

  for (subject_idx in 1:n_test_subjects) {
    cat("Test subject", subject_idx, "/", n_test_subjects, "\n"); flush.console()

    results_list[[subject_idx]] <- fit_maps(
      cm_result$candidate_maps,
      cm_result$full_hrf_grid,
      session_data_test45,  # Using TEST data (45 matched)
      subject_idx
    ) %>%
  ...
  output_file <- file.path(output_dir_created, paste0("results_cm_poc_test45_gambling_", run_names[r], "_weighted.rds"))
  saveRDS(results_combined, output_file)
  cat("\n*** Saved test results to:", output_file, "***\n"); flush.console()

  rm(regularize_result, cm_result, results_list, results_combined, session_data_original, session_data_test45); gc()
}
