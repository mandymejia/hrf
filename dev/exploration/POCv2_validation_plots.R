...
results_dir <- "/Volumes/LaCie/root/hrf_adaptation/validation_bonferroni_0.05_smoothed_RSS/POCv2"
...
for (run in run_names) {
  ...
  # Load POCv2 results
  results_test_matched <- readRDS(file.path(results_dir, paste0("results_POCv2_test_matched_gambling_", run, "_weighted.rds")))
  results_retest <- readRDS(file.path(results_dir, paste0("results_POCv2_retest_gambling_", run, "_weighted.rds")))
  ...
  plot_dir <- "~/Documents/Github/hrf-z/dev/exploration/plots/POCv2_validation"
  ...
  p_hist <- ggplot(best_combined, aes(x = candidate_id, fill = run)) +
    ...
    labs(title = paste0("POCv2: Distribution of Best Candidate Maps (", run, ")"),
         x = "Candidate ID",
         y = "Number of Subjects",
         fill = "") +
    theme_minimal()

  ggsave(file.path(plot_dir, paste0("POCv2_candidate_hist_", run, ".png")), p_hist, width = 6, height = 4, dpi = 300)
  ...
  p_candidate <- ggplot(comparison, aes(x = candidate_id_test, y = candidate_id_retest)) +
    ...
    labs(title = paste0("POCv2 Test vs Retest: Candidate ID (", run, ")"),
         subtitle = paste0("r = ", round(cor_candidate, 3), " (n = ", nrow(comparison), ")"),
         x = "Test Candidate ID", y = "Retest Candidate ID") +
    theme_minimal()

  ggsave(file.path(plot_dir, paste0("POCv2_candidate_id_", run, ".png")), p_candidate, width = 6, height = 6, dpi = 300)
  ...
  p_a1 <- ggplot(comparison, aes(x = a1_offset_test, y = a1_offset_retest)) +
    ...
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
    labs(title = paste0("POCv2 Test vs Retest: a1 Offset (", run, ")"),
         subtitle = paste0("r = ", round(cor_a1, 3), " (n = ", nrow(comparison), ")"),
         x = "Test a1 Offset", y = "Retest a1 Offset") +
    theme_minimal()

  ggsave(file.path(plot_dir, paste0("POCv2_a1_offset_", run, ".png")), p_a1, width = 6, height = 6, dpi = 300)
  ...
  p_b1 <- ggplot(comparison, aes(x = b1_offset_test, y = b1_offset_retest)) +
    ...
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
    labs(title = paste0("POCv2 Test vs Retest: b1 Offset (", run, ")"),
         subtitle = paste0("r = ", round(cor_b1, 3), " (n = ", nrow(comparison), ")"),
         x = "Test b1 Offset", y = "Retest b1 Offset") +
    theme_minimal()

  ggsave(file.path(plot_dir, paste0("POCv2_b1_offset_", run, ".png")), p_b1, width = 6, height = 6, dpi = 300)
  ...
  p_t2p <- ggplot(comparison, aes(x = t2p_test, y = t2p_retest)) +
    ...
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
    labs(title = paste0("POCv2 Test vs Retest: Time to Peak (", run, ")"),
         subtitle = paste0("r = ", round(cor_t2p, 3), " (n = ", nrow(comparison), ")"),
         x = "Test t2p (sec)", y = "Retest t2p (sec)") +
    theme_minimal()

  ggsave(file.path(plot_dir, paste0("POCv2_t2p_", run, ".png")), p_t2p, width = 6, height = 6, dpi = 300)

  # Plot 5: Scatter plot - fwhm
  p_fwhm <- ggplot(comparison, aes(x = fwhm_test, y = fwhm_retest)) +
    geom_jitter(alpha = 0.5, size = 3, width = 0.05, height = 0.05) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
    labs(title = paste0("POCv2 Test vs Retest: FWHM (", run, ")"),
         subtitle = paste0("r = ", round(cor_fwhm, 3), " (n = ", nrow(comparison), ")"),
         x = "Test FWHM (sec)", y = "Retest FWHM (sec)") +
    theme_minimal()

  ggsave(file.path(plot_dir, paste0("POCv2_fwhm_", run, ".png")), p_fwhm, width = 6, height = 6, dpi = 300)
