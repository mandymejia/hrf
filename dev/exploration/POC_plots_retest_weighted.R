...
session_data_LR_test45 <- readRDS("~/Documents/Github/hrf-z/dev/exploration/fixtures/test_matched_session_data_tfMRI_GAMBLING_LR.rds")
...
subjects_LR_test45 <- sapply(session_data_LR_test45[["BOLD_files"]], extract_subject_id)
subjects_LR_retest <- sapply(session_data_LR_retest[["BOLD_files"]], extract_subject_id)

cat("Test45 LR has", length(subjects_LR_test45), "subjects\n")
cat("Retest LR has", length(subjects_LR_retest), "subjects\n")
cat("First 10 retest subjects:", head(subjects_LR_retest, 10), "\n\n")

# Load results (F-weighted)
lr_test45 <- readRDS("~/Documents/Github/hrf-z/dev/exploration/poc_results/results_cm_poc_test45_gambling_LR_weighted.rds")
lr_retest <- readRDS("~/Documents/Github/hrf-z/dev/exploration/poc_results/results_cm_poc_retest_gambling_LR_weighted.rds")

cat("Test45 LR results has", length(unique(lr_test45$subject_id)), "unique subject_ids\n")
...
lr_test45$actual_subject_id <- subjects_LR_test45[lr_test45$subject_id]
lr_retest$actual_subject_id <- subjects_LR_retest[lr_retest$subject_id]

# Test: verify no NAs in actual_subject_id
stopifnot(sum(is.na(lr_test45$actual_subject_id)) == 0)
...
best_lr_test45_compare <- best_lr_test45 %>%
...
test_retest_comparison <- inner_join(best_lr_test45_compare, best_lr_retest_compare, by = "actual_subject_id")
...
best_lr_test45$run <- "Test"
best_lr_retest$run <- "Retest"
best_combined <- bind_rows(best_lr_test45, best_lr_retest)
...
p_hist <- ggplot(best_combined, aes(x = candidate_id, fill = run)) +
  ...
  labs(title = "Distribution of Best Candidate Maps: Test vs Retest (LR, F-weighted)",
       x = "Candidate ID (1-25)",
...
plot_dir <- "~/Documents/Github/hrf-z/dev/exploration/plots/poc_retest_weighted"
if (!dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE)

# Save histogram
ggsave(file.path(plot_dir, "test_retest_candidate_hist_LR_weighted.png"), p_hist, width = 6, height = 4, dpi = 300)
...
p_candidate <- ggplot(test_retest_comparison, aes(x = candidate_id_test, y = candidate_id_retest)) +
  ...
  labs(title = "Test vs Retest: Best Candidate ID per Subject (LR, F-weighted)",
       subtitle = paste0("r = ", round(cor_candidate, 3), " (n = ", nrow(test_retest_comparison), ")"),
       x = "Test Candidate ID", y = "Retest Candidate ID") +
  theme_minimal()

ggsave(file.path(plot_dir, "test_retest_candidate_id_LR_weighted.png"), p_candidate, width = 6, height = 6, dpi = 300)
...
p_a1 <- ggplot(test_retest_comparison, aes(x = a1_offset_test, y = a1_offset_retest)) +
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  labs(title = "Test vs Retest: Best a1 Offset per Subject (LR, F-weighted)",
       subtitle = paste0("r = ", round(cor_a1, 3), " (n = ", nrow(test_retest_comparison), ")"),
       x = "Test a1 Offset", y = "Retest a1 Offset") +
  theme_minimal()

ggsave(file.path(plot_dir, "test_retest_a1_offset_LR_weighted.png"), p_a1, width = 6, height = 6, dpi = 300)
...
p_b1 <- ggplot(test_retest_comparison, aes(x = b1_offset_test, y = b1_offset_retest)) +
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  labs(title = "Test vs Retest: Best b1 Offset per Subject (LR, F-weighted)",
       subtitle = paste0("r = ", round(cor_b1, 3), " (n = ", nrow(test_retest_comparison), ")"),
       x = "Test b1 Offset", y = "Retest b1 Offset") +
  theme_minimal()

ggsave(file.path(plot_dir, "test_retest_b1_offset_LR_weighted.png"), p_b1, width = 6, height = 6, dpi = 300)
