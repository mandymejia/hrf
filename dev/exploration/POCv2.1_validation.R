...
fit_maps_v2 <- function(candidate_maps, full_hrf_grid, session_data, subject_idx) {
  ...
  for (i in 1:length(candidate_maps)) {
    ...
    # V2.1: Use Fstat^4 weighting to emphasize most important voxels more
    weighted_RSS_per_candidate[i] <- sum(voxel_rss * Fstat_vector^4, na.rm = TRUE) /
      sum(Fstat_vector^4, na.rm = TRUE)
  }
...
# HPC paths
output_dir <- "/N/project/hrf_adaptation/validation_bonferroni_0.05_smoothed_RSS/POCv2.1"
...
for(r in runs_to_process){
  ...
  for(data_type in data_types){
    ...
    output_file <- safe_path(output_dir, paste0("results_POCv2.1_", data_type, "_gambling_", run_names[r], "_weighted.rds"))
