...
# NOTE: snap_to_grid_t2p_fwhm removed - using inverse_lookup instead
...
create_population_avg <- function(root_dir, sess_name) {
  ...
  # V4: Also exclude edge HRFs (convex set) so offsets don't push out of bounds
  cat("Step 5: Computing population averages (excluding edge HRFs for convex set)...\n")

  # Define edge HRFs to exclude:
  # a1=3 pairs: (3, 0.5), (3, 0.75), (3, 1)
  # High-end edges: (12, 0.75), (11, 1), (10, 1.25)
  is_edge_hrf <- function(a1, b1) {
    (a1 == 3) |
    (a1 == 12 & b1 == 0.75) |
    (a1 == 11 & b1 == 1) |
    (a1 == 10 & b1 == 1.25)
  ...
  n_edge_excluded <- sum(is_edge_hrf(best_params_df$a1, best_params_df$b1) &
                          best_params_df$mask == TRUE &
                          best_params_df$voxel %in% pop_mask_voxels)
  cat("Excluding", n_edge_excluded, "observations with edge HRFs\n")

  pop_avg <- best_params_df %>%
    filter(
      mask == TRUE,                    # Subject mask
      voxel %in% pop_mask_voxels,      # Population mask
      !is_edge_hrf(a1, b1)             # Exclude edge HRFs (convex set)
    ) %>%
    group_by(voxel) %>%
    summarize(
  ...
  # Step 6: Convert averaged t2p/fwhm back to CONTINUOUS a1/b1 (NO SNAPPING)
...
# Run it and save (v4 uses continuous a1/b1 + convex set)
result <- create_population_avg(fit_results_root_dir, "GAMBLING_LR_1084s")
saveRDS(result, file.path(fixture_dir, "pop_avg_v4_tfMRI_GAMBLING_LR_1084s.rds"))
...
# NOTE: snap_to_grid removed - no longer snapping after offsets

# create_candidate_maps_v4: uses CONTINUOUS a1/b1 (NO SNAPPING)
create_candidate_maps_v4 <- function(pop_avg_result) {
  ...
  # Offsets (more granular)
  a1_offsets <- c(-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2)
  b1_offsets <- c(-0.5, -0.375, -0.25, -0.125, 0, 0.125, 0.25, 0.375, 0.5)
...
# fit_maps_v4: returns t2p/fwhm in results (same as v2, works with continuous values)
fit_maps_v4 <- function(candidate_maps, full_hrf_grid, session_data, subject_idx) {
...
# HPC paths
output_dir <- "/N/project/hrf_adaptation/validation_bonferroni_0.05_smoothed_RSS/POCv4"
...
for(r in runs_to_process){
  ...
  # Load pop_avg from fixtures (v4: uses CONTINUOUS a1/b1 + convex set)
  pop_avg_file <- file.path(fixture_dir, paste0("pop_avg_v4_", sess_name, ".rds"))
  cat("Loading pop_avg from", pop_avg_file, "\n"); flush.console()
  pop_avg_result <- readRDS(pop_avg_file)

  # Create candidate maps (v4: NO SNAPPING, continuous values)
  cat("Creating candidate maps (v4: no snapping)...\n"); flush.console()
  cm_result <- create_candidate_maps_v4(pop_avg_result)
  ...
  for(data_type in data_types){
    ...
    for (subject_idx in 1:n_subjects) {
      ...
      results_list[[subject_idx]] <- fit_maps_v4(
    ...
    output_file <- safe_path(output_dir, paste0("results_POCv4_", data_type, "_gambling_", run_names[r], "_weighted.rds"))
