...
n_chunks <- 10  # Split subjects into 5 chunks per run
...
for(r in 1:2){
  ...
  save_path <- safe_path(save_dir, paste0("regularize_allHRFs_result_", sess, "_", c(length(session_data$BOLD_files)), "s.rds"))

  n_subjects <- length(session_data$BOLD_files)
  chunk_size <- ceiling(n_subjects / n_chunks)

  for (chunk_id in 1:n_chunks) {
    job_index <- job_index + 1  # <- SLURM
    if (!is.na(array_id) && job_index != array_id) next  # <- SLURM

    # Calculate subject range for this chunk
    start_idx <- (chunk_id - 1) * chunk_size + 1
    end_idx <- min(chunk_id * chunk_size, n_subjects)
    subject_range <- start_idx:end_idx

    cat("\n********** Processing", sess, "- Chunk", chunk_id, "(subjects", start_idx, "to", end_idx, ") **********\n"); flush.console()

    # Load regularize_result and create candidate maps (once per chunk)
    regularize_result <- readRDS(save_path)
    cm_result <- create_candidate_maps(regularize_result)

    # Collect results for subjects in this chunk
    results_list <- list()

    for (subject_idx in subject_range) {
      cat("Subject", subject_idx, "/", n_subjects, "\n"); flush.console()
      ...
      results_list[[subject_idx]] <- fit_maps(
        ...
        mutate(subject_id = subject_idx)  # Preserves original index
    }

    # Save this chunk
    output_file <- safe_path(output_dir, paste0("results_cm_poc_gambling_", run_names[r], "_chunk_", chunk_id, ".rds"))
    saveRDS(results_list, output_file)
    cat("\n*** Saved chunk to:", output_file, "***\n"); flush.console()

    rm(regularize_result, cm_result, results_list); gc()
  }
}