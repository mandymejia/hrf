...
fit_maps <- function(candidate_maps, full_hrf_grid, session_data, subject_idx) {
  ...
  for (i in 1:length(candidate_maps)) {
    ...
    # Linear F-stat weighting (original)
    # weighted_RSS_per_candidate[i] <- sum(voxel_rss * abs(Fstat_vector), na.rm = TRUE) /
    #   sum(abs(Fstat_vector), na.rm = TRUE)

    # Quadratic F-stat weighting (dramatically upweights high-activation voxels)
...
save_dir <- "/N/project/hrf_adaptation/validation_bonferroni_0.05_smoothed/regularize_allHRFs"

# # Local paths (comment out for HPC execution)
# save_dir <- "/Volumes/LaCie/root/hrf_adaptation/validation_bonferroni_0.05/regularize_allHRFs_winnerc"
output_dir <- "~/Documents/Github/hrf-z/dev/exploration/poc_results/weighted_smoothed"
...
for(r in runs_to_process){
  ...
  for(data_type in data_types){
    ...
    output_dir_created <- safe_path(output_dir)
    output_file <- file.path(output_dir_created, paste0("results_cm_poc_", data_type, "_gambling_", run_names[r], "_weighted.rds"))
  ...
  rm(regularize_result, cm_result, session_data_original); gc()
}
